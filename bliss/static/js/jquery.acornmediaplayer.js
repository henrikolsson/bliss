		...
		var acornPlayer = function() {
			...
			*/
			...
			var seekUpdate = function() {
                ...
                if (acorn.$self[0].getAttribute('data-new-location')) {
                    currenttime = parseInt(currenttime) + parseInt(acorn.$self[0].getAttribute('data-new-location'));
                }
			...
			var startSeek = function(e, ui) {					
                ...
                var newUrl = acorn.$self[0].currentSrc.replace(/seek=[0-9]+/, "seek=" + seekLocation);
				console.log(seekLocation);
                console.log(newUrl);
                acorn.$self[0].setAttribute('data-new-location', seekLocation);
                acorn.$self[0].src = newUrl;
                acorn.$self[0].load()
                acorn.$self[0].play()
			...
			var updateSeek = function() {
                ...
                if (!isFinite(duration)) {
                    duration = acorn.$self[0].getAttribute('data-duration');
				...
				acorn.$self.bind('loadedmetadata', function() {
					...
					var t = window.setInterval(function() {
								if (acorn.$self[0].readyState > 0) {
									loadedMetadata = true;
                                    var foo = acorn.$self[0].getAttribute('data-new-location');
                                    console.log("foo: "  +foo);
                                    if (foo)
				                        acorn.$self[0].currentTime = foo;
